...
export function Minesweeper() {
  ...
  return (
    <div className="h-full bg-[#ECE9D8] p-2 flex flex-col items-center text-black">
      <div className="bg-[#ECE9D8] border-2 border-t-gray-600 border-l-gray-600 border-b-white border-r-white p-1 flex items-center gap-3 mb-2">
        <div className="w-12 bg-black text-red-500 font-mono text-lg text-center">
          {String(MINES - flagged).padStart(3, "0")}
        </div>
        <button
          onClick={init}
          className="w-8 h-8 bg-[#ECE9D8] border-2 border-t-white border-l-white border-b-gray-600 border-r-gray-600 text-xl"
        >
          {over ? "😵" : won ? "😎" : "🙂"}
        </button>
        <div className="w-12 bg-black text-red-500 font-mono text-lg text-center">
      ...
      <div className="border-2 border-t-gray-600 border-l-gray-600 border-b-white border-r-white p-1 bg-[#ECE9D8]">
        {grid.map((row, y) => (
          <div key={y} className="flex">
            {row.map((c, x) => (
              <button
                ...
                className={`w-6 h-6 text-sm font-bold flex items-center justify-center ${
                  c.revealed
                    ? "bg-[#ECE9D8] border border-gray-500"
                    : "bg-[#ECE9D8] border-2 border-t-white border-l-white border-b-gray-600 border-r-gray-600"
                }`}
                style={{ color: colorFor(c.count) }}
              >
